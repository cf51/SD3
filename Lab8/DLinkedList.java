...
public class DLinkedList {
	...
	public  void insertionSort(){
		...
		if (headNode != null && headNode.nextNode != null) {	//if the current head node and the next one is not null then it runs 
			
			Node current = headNode.nextNode; //current is the node after the head node 
			
			while(current != null) { //runs while current isn't null 
				
				Node n = current.nextNode; //node n is the next current node 
				
				Node p = current.prevNode; //node p is the previous node 
				
				if(p.value > current.value) { //if the value of p is greater than the current value 
					
					int temp = delete(current); //delete the current 
					
					while(p != null && p.value > temp) {
						
						p = p.prevNode; //move it to the new position 
						
					}
					
					insertAfter(p, temp); //insert back into the correct position 
